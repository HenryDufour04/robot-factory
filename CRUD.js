...
async function start() {
    ...
    if (action === 'Create') {
        ...
        if (friend === 'N') 
        {
            ...
            friend = true
            killer = false
            console.log('Yay a friend robot')
            serialNumber = await ask('What is the serial number?')
            date = new Date()
        }
        ...
        let updateTarget = await ask('What is the name of the robot you want to update?   ')
        let updateField = await ask('What field do you want to update?   ')
        let update = await ask('Enter a new value   ')

        await Robot.updateOne({ robotName: updateTarget }, { $set: { [updateField]: update } })
        console.log('Your robot has been updated!')

    } else if (action === 'Delete') {
        let allRobots = await Robot.find({})
        console.log(allRobots)
        let target = await ask('what is the name of the entry do you want to delete?   ')
        await Robot.deleteOne({ robotName: target })
